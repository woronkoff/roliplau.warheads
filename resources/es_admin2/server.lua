	...
	TriggerEvent('es:getPlayerFromId', source, function(user)
		TriggerEvent('es:canGroupTarget', user.getGroup(), "admin", function(available)
			if available or user.getGroup() == "superadmin" then
				...
				TriggerClientEvent('chatMessage', Source, "SYSTEM", {255, 0, 0}, "Hey você não tem permissão para isso")
			end
		end)
	end)
end)
				...
				TriggerEvent('es:canGroupTarget', user.getGroup(), target.getGroup(), function(canTarget)
					if canTarget and available then
						...
						if type == "ban" then
							...
							DropPlayer(id, GetConvar("es_admin_banreason", "Você foi emrabado"))
						end
					else
						if not available then
							TriggerClientEvent('chatMessage', Source, 'SYSTEM', {255, 0, 0}, "Esse Grupo não tem essa permissão.")
						else
							TriggerClientEvent('chatMessage', Source, 'SYSTEM', {255, 0, 0}, "Permissão Negada.")
						end
					end
				end)
			end)
		...
		TriggerEvent('es:canGroupTarget', user.getGroup(), "admin", function(available)
			...
			if t == "group" then
				if(GetPlayerName(USER) == nil)then
					TriggerClientEvent('chatMessage', source, 'SYSTEM', {255, 0, 0}, "Jogador não encontrado")
				else
					TriggerEvent("es:getAllGroups", function(groups)
						if(groups[GROUP])then
							TriggerEvent("es:setPlayerData", USER, "group", GROUP, function(response, success)
								...
								TriggerClientEvent('chatMessage', -1, "CONSOLE", {0, 0, 0}, "Grupo de ^2^*" .. GetPlayerName(tonumber(USER)) .. "^r^0 foi setado para ^2^*" .. GROUP)
							end)
						else
